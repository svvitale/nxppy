...
class nxppyTests(unittest.TestCase):
    ...
    @TestDecorator.either_or('detect')
    def test_detect_mifare_present(self):
        """Test that we can read the UID from a present Mifare card. Either this test or
        the "absent" test below will pass, but never both."""
        import nxppy
        self.assertIsInstance(nxppy.read_mifare(), str, "Card UID is not a string")

    @TestDecorator.either_or('detect')
    def test_detect_mifare_absent(self):
        """Test that no card results in a None response. Either this test or the "present"
        test above will pass, but never both."""
        import nxppy
